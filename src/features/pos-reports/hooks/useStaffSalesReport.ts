...
export const staffSalesReportKeys = {
  ...
  byRange: (from: string, to: string) =>
    ['posReports', 'byStaff', from, to] as const,
};

/**
 * Reporte de ventas por vendedor en un rango. `enabled` debe pasarse
 * SIEMPRE: solo se ejecuta cuando el usuario es super_admin Y el rango
 * es válido (el back devuelve 403 si llamamos como admin normal).
 */
export function useStaffSalesReport(
  dateFrom: string,
  dateTo: string,
  enabled: boolean,
) {
